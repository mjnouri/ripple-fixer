...
echo $DATEANDTIME >> /home/centos/website-checker/log

nc -z $RIPPLEHOST 22
if [[ $? -eq 0 ]]
then
  echo "Ripplestone server is reachable on SSH." >> /home/centos/website-checker/log
else
  echo "Ripplestone server is unreachable on SSH. Check status of server." >> /home/centos/website-checker/log
  aws sns publish --topic-arn arn:aws:sns:us-east-1:123:Me --message "$SERVERUNAVAILABLE" >> /home/centos/website-checker/log
  echo "--------------------------------------" >> /home/centos/website-checker/log
  exit 1
fi
...
then
  echo $UPMESSAGE >> /home/centos/website-checker/log
  exit 0
else
  echo $RIPPLEDOWN >> /home/centos/website-checker/log
...
then
  echo $RIPPLERESTARTED >> /home/centos/website-checker/log
  aws sns publish --topic-arn arn:aws:sns:us-east-1:123:Me --message "$RIPPLERESTARTED" >> /home/centos/website-checker/log
  exit 0
else
  echo $RIPPLENOTRESTARTED >> /home/centos/website-checker/log
  aws sns publish --topic-arn arn:aws:sns:us-east-1:123:Me --message "$RIPPLENOTRESTARTED" >> /home/centos/website-checker/log
  exit 1
fi